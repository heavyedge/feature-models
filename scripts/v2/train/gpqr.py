...
if has_validation:
    ...
    if not study.trials:
        baseline_num_latents = int(
            np.clip(
                HYPERPARAMETER_DEFAULTS["num_latents"],
                2,
                sum(num_quantiles),
            )
        )
        baseline_params = {
            ...
            "num_latents": baseline_num_latents,
            "num_central_latents": int(
                np.clip(
                    HYPERPARAMETER_DEFAULTS["num_central_latents"],
                    1,
                    min(len(TARGET), baseline_num_latents - 1),
                )
            ),
        }
        study.enqueue_trial(
...
if "num_central_latents" not in optimized_hyperparameters:
