...
def objective(trial):
    ...
    num_latents = trial.suggest_int("num_latents", 2, sum(num_quantiles))
    num_central_latents = trial.suggest_int(
        "num_central_latents",
        1,
        min(len(TARGET), num_latents - 1),
    )
...
if has_validation:
    ...
    if not study.trials:
        baseline_num_latents = int(np.clip(DEFAULT_NUM_LATENTS, 2, sum(num_quantiles)))
        baseline_params = {
            ...
            "num_central_latents": min(len(TARGET), baseline_num_latents - 1),
        }
...
best_num_central_latents = best_trial.params["num_central_latents"]
logger.info(
