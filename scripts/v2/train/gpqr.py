...
from models.v2.feature_models import gpqr as model_module
from models.v2.feature_models import load as load_module
from models.v2.feature_models import scale as scaler_module
from models.v2.feature_models.likelihoods import MultioutputCenterGapQuantilesLikelihood
...
from scripts.v2.train.save import save_gpqr

logger = configure_logging(__name__)

torch.manual_seed(42)

HYPERPARAMETER_DEFAULTS = PRIOR_HYPERPARAMETER_DEFAULTS | {
    "num_latents": None,
...
quantiles = [
    torch.tensor(args.quantiles, dtype=torch.float32, device=device)
    for _ in range(len(TARGET))
]
central_quantile_idx = [
    np.argmin(np.abs(q.detach().cpu().numpy() - 0.5)) for q in quantiles
]
num_quantiles = [len(q) for q in quantiles]
num_lower_quantiles = central_quantile_idx

HYPERPARAMETER_DEFAULTS["num_latents"] = sum(num_quantiles)
HYPERPARAMETER_DEFAULTS["num_central_latents"] = len(TARGET)
...
def objective(trial):
    ...
    hyperparameters["num_latents"] = int(
        np.clip(hyperparameters["num_latents"], 2, sum(num_quantiles))
    )
    if "num_latents" in optimized_hyperparameters:
        hyperparameters["num_latents"] = trial.suggest_int(
            "num_latents", 2, sum(num_quantiles)
        )
    max_central_latents = min(len(TARGET), hyperparameters["num_latents"] - 1)
    hyperparameters["num_central_latents"] = int(
        np.clip(
            hyperparameters["num_central_latents"],
            1,
            max_central_latents,
        )
    )
    if "num_central_latents" in optimized_hyperparameters:
        hyperparameters["num_central_latents"] = trial.suggest_int(
            "num_central_latents",
            1,
            max_central_latents,
        )
...
default_hyperparameters["num_latents"] = int(
    np.clip(default_hyperparameters["num_latents"], 2, sum(num_quantiles))
)
baseline_params = prior_baseline(args)
baseline_params["num_latents"] = default_hyperparameters["num_latents"]
baseline_params["num_central_latents"] = int(
    np.clip(
        default_hyperparameters["num_central_latents"],
        1,
        min(len(TARGET), baseline_params["num_latents"] - 1),
    )
)
...
max_central_latents = min(len(TARGET), best_hyperparameters["num_latents"] - 1)
if "num_central_latents" not in selected_hyperparameters:
    best_hyperparameters["num_central_latents"] = int(
        np.clip(
            HYPERPARAMETER_DEFAULTS["num_central_latents"],
            1,
            max_central_latents,
        )
    )
...
best_num_central_latents = best_hyperparameters["num_central_latents"]
logger.info(
