...
test: _temp/test-X.npy
	python3 -c "from model.load import gpr_H; gpr_H()[0](np.load('$<'))"
	python3 -c "from model.load import gpr_b; gpr_b()[0](np.load('$<'))"
	python3 -c "from model.load import gpr_phi; gpr_phi()[0](np.load('$<'))"
	python3 -c "from model.load import gpqr_H; gpqr_H()[0](np.load('$<'))"
	python3 -c "from model.load import gpqr_phi; gpqr_phi()[0](np.load('$<'))"
