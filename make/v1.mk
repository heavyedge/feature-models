...
_temp/v1/cv.prior_mean.pt: scripts/v1/train/prior_mean.py _temp/v1/Xtrain.csv _temp/v1/ytrain.csv _temp/v1/Xval.csv _temp/v1/yval.csv \
$(SCRIPTS_v1)
	PYTHONPATH=. $(GPU_PYTHON) $(wordlist 1,5,$^) --index-col 0 --batch-col 0 --model PriorMean --num-epochs $(N_EPOCHS_v1) -o $@
...
_temp/v1/cv.gpr.pt _temp/v1/gpr.study-name &: scripts/v1/train/gpr.py _temp/v1/Xtrain.csv _temp/v1/ytrain.csv _temp/v1/Xval.csv _temp/v1/yval.csv _temp/v1/cv.prior_mean.pt \
$(SCRIPTS_v1)
	mkdir -p benchmarks
	study_name="v1/gpr/$$(date +%Y%m%dT%H%M%S%N%z)"
	printf '%s\n' "$$study_name" > _temp/v1/gpr.study-name
	PYTHONPATH=. $(GPU_PYTHON) $(wordlist 1,6,$^) --index-col 0 --batch-col 0 --model GPR --num-epochs $(N_EPOCHS_v1) --optimize-hyperparameters lengthscale_prior_loc lengthscale_prior_scale --n-trials=$(N_TRIALS_v1) --n-startup-trials=$(N_STARTUP_TRIALS_v1) --storage=$(OPTUNA_DB) --study-name="$$study_name" -o _temp/v1/cv.gpr.pt
...
_temp/v1/gpr.study-name $(SCRIPTS_v1)
	...
	PYTHONPATH=. $(GPU_PYTHON) $(wordlist 1,4,$^) --index-col 0 1 2 --model GPR --storage=$(OPTUNA_DB) --study-name="$$study_name" -o $@

## GPQR

_temp/v1/cv.gpqr_independent.pt _temp/v1/gpqr_independent.study-name &: scripts/v1/train/gpqr.py _temp/v1/Xtrain.csv _temp/v1/ytrain.csv _temp/v1/Xval.csv _temp/v1/yval.csv _temp/v1/cv.prior_mean.pt _temp/v1/cv.gpr.pt _temp/v1/gpr.study-name \
$(SCRIPTS_v1)
	mkdir -p benchmarks
	study_name="v1/gpqr_independent/$$(date +%Y%m%dT%H%M%S%N%z)"
	gpr_study_name="$$(cat _temp/v1/gpr.study-name)"
	printf '%s\n' "$$study_name" > _temp/v1/gpqr_independent.study-name
	PYTHONPATH=. $(GPU_PYTHON) $(wordlist 1,6,$^) --gpr-model=$(word 7,$^) --gpr-storage=$(OPTUNA_DB) --gpr-study-name="$$gpr_study_name" --index-col 0 --batch-col 0 --model GPQR_Independent --quantiles $(QUANTILES) --num-likelihood-samples $(N_LIKELIHOOD_SAMPLES) --batch-size $(GPQR_BATCH_SIZE_v1) --quantile-slope-lower-bound $(GPQR_QUANTILE_SLOPE_LOWER_BOUND_v1) --num-epochs $(N_EPOCHS_v1) --storage=$(OPTUNA_DB) --study-name="$$study_name" -o _temp/v1/cv.gpqr_independent.pt

_temp/v1/cv.gpqr_lmc.pt _temp/v1/gpqr_lmc.study-name &: scripts/v1/train/gpqr.py _temp/v1/Xtrain.csv _temp/v1/ytrain.csv _temp/v1/Xval.csv _temp/v1/yval.csv _temp/v1/cv.prior_mean.pt _temp/v1/cv.gpr.pt _temp/v1/gpr.study-name \
$(SCRIPTS_v1)
	mkdir -p benchmarks
	study_name="v1/gpqr_lmc/$$(date +%Y%m%dT%H%M%S%N%z)"
	gpr_study_name="$$(cat _temp/v1/gpr.study-name)"
	printf '%s\n' "$$study_name" > _temp/v1/gpqr_lmc.study-name
	PYTHONPATH=. $(GPU_PYTHON) $(wordlist 1,6,$^) --gpr-model=$(word 7,$^) --gpr-storage=$(OPTUNA_DB) --gpr-study-name="$$gpr_study_name" --index-col 0 --batch-col 0 --model GPQR_LMC --quantiles $(QUANTILES) --num-likelihood-samples $(N_LIKELIHOOD_SAMPLES) --batch-size $(GPQR_BATCH_SIZE_v1) --quantile-slope-lower-bound $(GPQR_QUANTILE_SLOPE_LOWER_BOUND_v1) --num-epochs $(N_EPOCHS_v1) --storage=$(OPTUNA_DB) --study-name="$$study_name" -o _temp/v1/cv.gpqr_lmc.pt

_temp/v1/cv.gpqr_cglmc.pt _temp/v1/gpqr_cglmc.study-name &: scripts/v1/train/gpqr.py _temp/v1/Xtrain.csv _temp/v1/ytrain.csv _temp/v1/Xval.csv _temp/v1/yval.csv _temp/v1/cv.prior_mean.pt _temp/v1/cv.gpr.pt _temp/v1/gpr.study-name \
$(SCRIPTS_v1)
	mkdir -p benchmarks
	study_name="v1/gpqr_cglmc/$$(date +%Y%m%dT%H%M%S%N%z)"
	gpr_study_name="$$(cat _temp/v1/gpr.study-name)"
...
models/v1/feature_models/gpr.pt _temp/v1/gpr.study-name _temp/v1/gpqr_cglmc.study-name $(SCRIPTS_v1)
	mkdir -p $(@D)
	gpr_study_name="$$(cat _temp/v1/gpr.study-name)"
	study_name="$$(cat _temp/v1/gpqr_cglmc.study-name)"
	PYTHONPATH=. $(GPU_PYTHON) $(wordlist 1,4,$^) --gpr-model=$(word 5,$^) --gpr-storage=$(OPTUNA_DB) --gpr-study-name="$$gpr_study_name" --index-col 0 1 2 --model GPQR_CenterGapLMC --quantiles $(QUANTILES) --num-likelihood-samples $(N_LIKELIHOOD_SAMPLES) --batch-size $(GPQR_BATCH_SIZE_v1) --quantile-slope-lower-bound $(GPQR_QUANTILE_SLOPE_LOWER_BOUND_v1) --num-epochs $(N_EPOCHS_v1) --storage=$(OPTUNA_DB) --study-name="$$study_name" -o $@

# Model selection

_temp/v1/cv.gpr.Xtest.csv: _temp/v1/Xtest.csv _temp/v1/cv.prior_mean.pt _temp/v1/cv.gpr.pt $(SCRIPTS_v1)
	mkdir -p $(@D)
	$(GPU_PYTHON) -m models.v1.feature_models.predict-gpr $(wordlist 1,3,$^) --index-col 0 -o $@

benchmarks/v1/pinball_loss.gpr.csv: scripts/v1/model_selection/pinball_loss.py _temp/v1/cv.gpr.Xtest.csv _temp/v1/ytest.csv
	mkdir -p $(@D)
...
benchmarks/v1/gpr.Xpred_2D.csv: _temp/v1/Xpred_2D.csv $(SCRIPTS_v1) models/v1/feature_models/prior_mean.pt models/v1/feature_models/gpr.pt
	mkdir -p $(@D)
	$(GPU_PYTHON) -m models.v1.feature_models.predict-gpr $< --index-col 0 1 2 -o $@
